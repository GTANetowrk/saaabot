...
# ── Función para generar transcript ───────────────────────
async def generate_transcript(channel: discord.TextChannel) -> str:
    lines = []
    lines.append(f"SAAA TICKET TRANSCRIPT")
    lines.append(f"=" * 60)
    lines.append(f"Channel : #{channel.name}")
    lines.append(f"Topic   : {channel.topic or 'N/A'}")
    ...
    lines.append("=" * 60)
    lines.append("End of transcript — San Andreas Aviation Administration")
    return "\n".join(lines)
...
class TicketControlView(discord.ui.View):
    ...
    @discord.ui.button(label="📄  Transcript", style=discord.ButtonStyle.primary, custom_id="transcript")
...
"
        ...
        embed.set_footer(text="San Andreas Aviation Administration")
        ...
        # Enviar al usuario de forma privada
        await interaction.followup.send(
            content="✅ Transcript generated and saved to logs.",
            file=file,
            ephemeral=True
        )
    ...
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Generar transcript automáticamente al cerrar
        transcript_text = await generate_transcript(interaction.channel)
        filename = f"transcript-{interaction.channel.name}-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}.txt"
        transcript_bytes = transcript_text.encode("utf-8")
...
"
                f"Transcript saved automatically."
            ),
            color=0xC0392B
        )
        close_embed.set_footer(text="San Andreas Aviation Administration")

        # Enviar transcript a logs antes de borrar
        log_ch = discord.utils.get(interaction.guild.text_channels, name="ticket-logs")
        if log_ch:
            log_file = discord.File(
                fp=__import__("io").BytesIO(transcript_bytes),
            ...
            await log_ch.send(embed=close_embed, file=log_file)

        await interaction.channel.send(
            embed=discord.Embed(
                description="🔒 Ticket closing in **5 seconds**. Transcript saved to logs.",
                color=0xC0392B
            )
        )
...
@bot.event
async def on_ready():
    # Registrar views persistentes para que sobrevivan reinicios
    bot.add_view(TicketSelectView())
    bot.add_view(TicketControlView(opener_id=0))
